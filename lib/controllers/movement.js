...
module.exports = (socket, controller) => {
  ...
  controller.rpad.on('move', (event) => {
    ...
    let valueX = event.normx;
    valueX *= 10;
    valueX = Math.round(valueX);
    valueX /= 10;

    let valueY = event.normy;
    valueY *= 10;
    valueY = Math.round(valueY);
    valueY /= 10;

    const toEmit = `{"horizontal": ${-1 * valueX}, "vertical": ${valueY}}`;
    if (toEmit === prevHeadset) {
      return;
    }

    debug('%o', JSON.parse(toEmit));
    socket.emit('headset position', toEmit);
    prevHeadset = toEmit;
  });
