...
controller.rpad.on('move', (event) => {
  let xValue = event.normx;
  xValue *= 10;
  xValue = Math.round(xValue);
  xValue *= 10;
  xValue = Math.min(xValue, 100);
  xValue = Math.max(xValue, -100);

  let yValue = event.normy;
  yValue *= 10;
  yValue = Math.round(yValue);
  yValue *= 10;
  yValue = Math.min(yValue, 100);
  yValue = Math.max(yValue, -100);

  socket.emit('headset position', {
    horizontal: xValue,
    vertical: yValue,
  });
});
